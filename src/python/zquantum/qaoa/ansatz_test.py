...
from .ansatz import (
    build_farhi_qaoa_circuit_template,
    build_qaoa_circuit,
    build_qaoa_circuit_grads,
)
...
class TestQAOA(unittest.TestCase):
    def test_build_farhi_qaoa_circuit_template(self):
        ...
        correct_circuit_template = {
            ...
            "ansatz_kwargs": {
                "hamiltonians": [
                    {
                        "schema": SCHEMA_VERSION + "-qubit_op",
                        "terms": [
                            {
                                "pauli_ops": [{"qubit": 0, "op": "Z"}],
    ...
    def test_build_qaoa_ansatz_grads(self):
        ...
        problem_hamiltonian = generate_random_qubitop(
            num_qubits, num_terms, locality, max_coeff
        )
        ...
        grad_circuits, grad_factors = build_qaoa_circuit_grads(
            params, circuit_template["ansatz_kwargs"]["hamiltonians"]
        )
