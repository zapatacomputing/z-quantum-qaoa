...
from .circuit import run_circuit_and_get_distribution
...
def generate_random_k_sat_instance_and_get_seed_solutions(
    ...
    while True:
        ...
        print(f"mean value of random assignment = {mean_value}\n")
    ...
    if local_version:
        ...
        save_k_sat_instance(k_sat, "k_sat.json")
        with open("costs_and_ranks.json", 'w') as outfile:
            json.dump(costs_and_ranks, outfile)
        with open("seed_assignments_and_values.json", 'w') as outfile:
            json.dump(seed_assignments_and_values, outfile)
...
def solve_max_k_sat_by_cbqo(
    ...
    if not local_version:
        ...
        with open(costs_and_ranks, 'r') as infile:
            costs_and_ranks = json.load(costs_and_ranks)
        with open(seed_assignments_and_values, 'r') as infile:
            seed_assignments_and_values = json.load(seed_assignments_and_values)
    ...
    for i in range(num_variables):
        ...
        value_difference = new_value - value
